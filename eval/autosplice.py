...
class AutoSplice(Dataset):
    def __init__(self, root: str, forged_dir: str = "Forged_JPEG100"):
        self.root = Path(root)
        mask_dir = self.root / "Mask"
        self.entries = []
        n_skip = 0
        for p in sorted((self.root / forged_dir).glob("*.jpg")):
            base = p.stem.rsplit("_", 1)[0]
            mp = mask_dir / f"{base}_mask.png"
            if not mp.exists():
                n_skip += 1
                continue
            self.entries.append({"img": p, "mask": mp, "member": 1})
        for p in sorted((self.root / "Authentic").glob("*.jpg")):
            self.entries.append({"img": p, "mask": None, "member": 0})
        n_fake = sum(e["member"] == 1 for e in self.entries)
        n_real = sum(e["member"] == 0 for e in self.entries)
        print(f"AutoSplice[{forged_dir}]: fake={n_fake} (skipped {n_skip} w/o mask)  "
              f"real={n_real}", flush=True)
    ...
    def __getitem__(self, idx):
        e = self.entries[idx]
        img_t = _img_t(Image.open(e["img"]).convert("RGB"))
        if e["mask"] is None:
            mask_t = torch.zeros(1, IMG_SIZE, IMG_SIZE, dtype=torch.float32)
        else:
            m = Image.open(e["mask"]).convert("L")
...
@torch.no_grad()
@hydra.main(version_base=None, config_path="../configs/eval", config_name="autosplice")
def main(cfg: DictConfig):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = build_model(cfg.checkpoint, device, cfg.model_from_checkpoint)
    model.eval()

    ds = AutoSplice(cfg.data_root, cfg.forged_dir)
    ...
    for imgs, masks, member in tqdm(loader, desc="eval", unit="batch"):
        ...
        with torch.autocast("cuda"):
            res = model(imgs)
        probs = res["logits"].sigmoid().squeeze(1)
    ...
    print(f"  Pix.F1(fake, per-image)={b['f1_per_image_fake']*100:.2f}  "
          f"IoU(fake, per-image)={b['iou_per_image_fake']*100:.2f}")
    print(f"Written: {cfg.out}")
